...
def item_quota_rando(rooms, nitems=6):
	...
	#while len(current_items) < nitems:
		...
		for room_name in rooms_to_place:
			...
			paths_through, _, _ = room_graph.BFS_items(chosen_entrance, None, current_wildcards, current_items, current_assignments, fixed_items)
			filter_paths(paths_through, chosen_entrance, current_wildcards, current_items, room_dummy_exits)
			#print paths_through
			...
			if len(paths_through) > 0:
				...
				#print room[0].name
				#print exits_to_connect
				#print chosen_direction + ":" + chosen_exit, room_direction + ":" + chosen_entrance
